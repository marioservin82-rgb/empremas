...
export default function CobrosDia() {
  ...
  async function cargar(f, sucId) {
    ...
    try {
      ...
      setResumen(await apiFetch(`/api/cobros/resumen-dia${query}`));
    } catch (err) {
      ...
      <div className="w-full max-w-2xl">
        <div className="flex items-center justify-between py-6 print:hidden">
          <div>
            ...
            <h1 className="text-2xl font-bold text-navy">Crédito cobrado hoy</h1>
          </div>
          {resumen && (
            <button
        ...
        {cargando ? (
          ...
          <div className="reporte-imprimible rounded-2xl bg-white p-6 shadow shadow-slate-200">
            ...
            <div className="mb-4 hidden print:block">
              <p className="text-xl font-bold">Crédito cobrado hoy</p>
            ...
            <div className="mb-6 rounded-2xl bg-slate-50 p-5 text-center print:border print:border-slate-300 print:bg-white">
              ...
              <p className="text-4xl font-extrabold text-navy">Gs {formatoGs.format(resumen.totalCobrado)}</p>
              <p className="mt-2 text-sm text-slate-500">
                {resumen.cantidadCobros} {resumen.cantidadCobros === 1 ? "cobro" : "cobros"}
              </p>
            </div>

            {resumen.cobros.length === 0 ? (
              <p className="text-sm text-slate-400">Sin cobros a crédito registrados en este período.</p>
            ) : (
                  ...
                  <div key={c.id} className="py-3 text-sm">
                    <div className="mb-1 flex items-center justify-between">
                      <span className="font-semibold text-slate-800">
                        {hora(c.creado_en)} · Recibo N° {c.numero_recibo}
                      </span>
                      <span className="font-bold text-navy">Gs {formatoGs.format(c.monto)}</span>
                    </div>
                    <p className="text-slate-500">Cliente: {c.cliente_nombre}</p>
                    <div className="mt-1 flex flex-wrap items-center gap-x-3 text-xs text-slate-400">
                      {c.pagos.length > 0 && (
                        <span>{c.pagos.map((p) => ETIQUETA_FORMA_PAGO[p.forma_pago]).join(" + ")}</span>
                      )}
                      <span>Cobrado por: {c.usuario_nombre}</span>
                    </div>
                  </div>
                ))}
              </div>
