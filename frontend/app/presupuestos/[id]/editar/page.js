...
export default function EditarPresupuesto() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/presupuestos/${id}`)
      .then(async (p) => {
        setNumero(p.numero);
        setListaPrecio(p.lista_precio);
        setVencimiento(p.vencimiento.slice(0, 10));
        setCarrito(
          p.items.map((i) => ({
        ...
        if (p.cliente_id) {
          const c = await apiFetch(`/api/clientes/${p.cliente_id}`).catch(() => null);
          if (c) setCliente(c);
        }
        setListo(true);
      })
  ...
  function cambiarCantidad(productoId, cantidad) {
    setCarrito((actual) => actual.map((i) => (i.productoId === productoId ? { ...i, cantidad: Number(cantidad) || 0 } : i)));
  }
  ...
  return (
    <main className="flex flex-1 flex-col items-center p-6">
      <div className="w-full max-w-2xl">
        <div className="py-6">
          <Link href={`/presupuestos/${id}`} className="text-sm font-medium text-slate-500 hover:text-slate-700">
            ← Volver
          </Link>
          <h1 className="text-2xl font-bold text-navy">Editar presupuesto N° {numero ?? "—"}</h1>
        </div>
            ...
            {LISTAS_PRECIO.map((l) => (
              <button
                ...
                className={`flex-1 rounded-xl py-3 font-bold transition ${
                  listaPrecio === l.valor ? "bg-navy text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                }`}
              >
                {l.etiqueta}
              </button>
