...
-- Nota de Credito / Debito electronica (SIFEN iTiDE 5 y 6). Siempre ajustan
-- una factura aprobada. NC por devolucion (motivo 1/2) reingresa stock; NC
-- total anula la venta y revierte el saldo si era a credito.
CREATE TYPE tipo_nota_electronica AS ENUM ('credito', 'debito');
CREATE TYPE estado_nota_electronica AS ENUM ('pendiente', 'enviado', 'aprobado', 'rechazado', 'error');
...
CREATE INDEX idx_notas_electronicas_empresa ON notas_electronicas (empresa_id);
CREATE INDEX idx_notas_electronicas_venta ON notas_electronicas (venta_id);
CREATE INDEX idx_nota_items_empresa ON nota_items (empresa_id);
CREATE INDEX idx_nota_items_nota ON nota_items (nota_id);
...
ALTER TABLE empresas ADD COLUMN reparaciones_habilitadas BOOLEAN NOT NULL DEFAULT false;
...
ALTER TABLE empresas ADD COLUMN siguiente_numero_reparacion INTEGER NOT NULL DEFAULT 1;
...
CREATE INDEX idx_reparaciones_empresa ON reparaciones (empresa_id);
CREATE INDEX idx_reparaciones_cliente ON reparaciones (cliente_id);
CREATE INDEX idx_reparaciones_estado ON reparaciones (empresa_id, estado);
...
ALTER TABLE empresas ADD COLUMN mascotas_habilitadas BOOLEAN NOT NULL DEFAULT false;
ALTER TABLE empresas ADD COLUMN siguiente_numero_internacion INTEGER NOT NULL DEFAULT 1;
...
CREATE INDEX idx_mascotas_empresa ON mascotas (empresa_id);
...
-- Citas: vinculo opcional a una mascota (tratamiento/analisis en
-- veterinaria) y un resultado de texto libre. NULL para una cita comun de
-- salon/barberia.
ALTER TABLE citas ADD COLUMN mascota_id UUID REFERENCES mascotas(id);
ALTER TABLE citas ADD COLUMN resultado TEXT;
CREATE INDEX idx_citas_mascota ON citas (mascota_id) WHERE mascota_id IS NOT NULL;

-- Ficha de vacunacion por mascota. fecha_proximo_refuerzo alimenta el
-- listado de alertas (mismo espiritu que clientesCumpleanos).
...
CREATE INDEX idx_vacunas_refuerzo ON vacunas (empresa_id, fecha_proximo_refuerzo) WHERE fecha_proximo_refuerzo IS NOT NULL;
...
-- Internacion (hospedaje/hospitalizacion): sin precio conocido de antemano
-- (depende de cuantos dias se quede) - al dar de alta se carga el total a
-- mano y "Cobrar" manda a Vender, mismo criterio que Reparaciones.
...
CREATE INDEX idx_internaciones_empresa ON internaciones (empresa_id);
CREATE INDEX idx_internaciones_mascota ON internaciones (mascota_id);
CREATE INDEX idx_internaciones_estado ON internaciones (empresa_id, estado);
